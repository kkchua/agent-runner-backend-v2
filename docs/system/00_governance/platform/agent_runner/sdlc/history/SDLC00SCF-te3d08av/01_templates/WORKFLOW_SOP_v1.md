...
Every SDLC initiative workflow MUST follow this seven-step pattern. This
pattern ensures quality through internal technical review, human approval,
and controlled promotion.

### Step Pattern Definition

1. **generate_<artifact>** (prompt) -- Generate the document from input.
   The workflow produces an initial draft document based on the input
   artifact.

2. **technical_critique** (prompt) -- Internal quality gate. A separate
   coder evaluates the generated document for feasibility, technical
   soundness, completeness, and alignment with input requirements.
   Produces a critique report with findings.

3. **address_critique** (prompt) -- Address findings from technical
   critique. The workflow updates the document in-place to resolve the
   findings identified in step 2. Adds a Critique Resolution section to
   the document summarizing how each finding was addressed.

4. **review_<artifact>** (prompt) -- Human approval gate. The human
   reviewer verifies that the critique was properly resolved and that the
   document meets standards. May approve or request changes.

5. **refine_<artifact>** (prompt, conditional) -- Refine based on review
   feedback. Only executed if the human reviewer requests changes. Updates
   the document based on reviewer feedback and returns to step 4.

6. **promote_<artifact>** (action) -- Promote document to approved status.
   Changes `lifecycle_status` from `draft` to `approved` in the document
   frontmatter.

7. **step_completion** (action) -- Finalize and notify. Records completion
   in the run log and notifies the next workflow or stakeholder.

### Step Pattern Enforcement

- Every initiative workflow MUST implement all seven steps in order.
- Steps 1-4 are mandatory. Step 5 is conditional (only on review
  feedback). Steps 6-7 are mandatory completion steps.
- The technical_critique step MUST be performed by a different coder
  instance than the generate step to ensure independent review.
- The address_critique step MUST produce a Critique Resolution section
  in the document.
- The refine loop (steps 4-5) has a maximum iteration budget (typically
  3). After exhausting the budget, the workflow fails.

### Step Pattern Variations

Some workflows have minor variations on this pattern:

- **sdlc_10_requirement_v1**: Uses a two-file promotion pattern (DRAFT-INIT
  to INIT-DOC). Steps 1-3 operate on the draft, then step 6 promotes by
  creating a new INIT-DOC file.

- **sdlc_80_review_v1**: Produces three artifacts (REV, MEM, CLOSE). Each
  artifact goes through the generate-critique-address sequence, then all
  three are promoted together in a single promote step.
...
If the technical critique identifies unresolvable defects:

1. Document the specific defects in the critique report.
2. If defects are not fixable through refinement, fail the workflow
   immediately without entering the refine loop.
3. Common unresolvable defects: wrong scope, invalid input data,
...
- Layer 2 Platform Constitution: METADATA_CONTRACT.md (inherited)
- Layer 1 Governance: METADATA_STANDARD.md, DOCUMENT_AUTHORITY.md (inherited)
