...
- Initiative ID (e.g., INIT-20260806-001)
...
- **Assumptions**: Items assumed to be true but not verified.

### 7. Constraints

Inherited and refined constraints from the draft initiative:
...
Refined dependency list with status:
...
Specific, testable success criteria linked to expected outcomes:
...
Section added by the address_critique step summarizing how findings
from the technical_critique step were resolved.
...
- The Critique Resolution section MUST be present after the
  address_critique step.
...
```
INIT-20260806-001_add-user-authentication.md
```
...
- **03_REQ_template.md** (SYS-03-RQ): The output produced by sdlc_20
  from this initiative document.

### Related Agent Contracts

- sdlc_10 uses its own prompts (no agent contract) to produce this
  document from the DRAFT-INIT.
- **AGENT-planner**: Consumes this INIT-DOC in sdlc_20 to produce the
  REQ-DOC.
