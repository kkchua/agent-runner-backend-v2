...
### Standard Step Pattern for Initiative Workflows

Every SDLC initiative workflow (sdlc_10 through sdlc_80) MUST follow
this standard step pattern. The pattern ensures consistent quality
gates, human review, and traceability across all workflow outputs.

1. **generate_<artifact>** (prompt) -- Generate the document from input.
   The workflow coder produces the initial document based on the input
   artifact and the template structure.

2. **technical_critique** (prompt) -- Internal quality gate. The workflow
   coder evaluates the generated document for feasibility, technical
   soundness, completeness, and adherence to the template structure. This
   is an automated self-review, not a human review. The output is a
   critique report listing findings and severity.

3. **address_critique** (prompt) -- Address findings from the technical
   critique step. The workflow coder updates the document in-place to
   resolve each finding. A Critique Resolution section is added to the
   document documenting how each finding was addressed.

4. **review_<artifact>** (prompt) -- Human approval gate. The workflow
   presents the document (including the Critique Resolution section) to
   the human reviewer. The reviewer verifies that the critique was
   properly resolved and the document meets standards. The reviewer may
   approve, request changes, or reject.

5. **refine_<artifact>** (prompt, conditional) -- Refine based on review
   feedback. This step executes only if the human reviewer requests
   changes. The workflow coder addresses the reviewer's feedback and
   updates the document. After refinement, the document re-enters the
   review step (step 4).

6. **promote_<artifact>** (action) -- Promote document to approved status.
   The workflow updates the document's `lifecycle_status` frontmatter
   from `draft` to `approved`. After promotion, the document is
   immutable.

7. **step_completion** (action) -- Finalize and notify. The workflow
   records completion metadata, updates the run status, and notifies
   downstream consumers that the artifact is ready.

### Step Pattern Rules

- Steps 1 through 3 are mandatory for every workflow run.
- Step 4 is mandatory; the workflow cannot proceed without human review.
- Step 5 is conditional; it only runs if the reviewer requests changes.
- Steps 6 and 7 are mandatory for successful completion.
- The refine loop (steps 4-5) has a maximum iteration budget
  (default: 2, configurable per workflow). After exhausting the budget,
  the workflow fails.
...
| template | Document is a template definition, not an instance. |
| changes_requested | Review identified fixable defects. |
| approved | All gates passed. Document is immutable. |

### Layer 3 lifecycle_status Extension

The value "template" is a Layer 3 extension to the Layer 1 lifecycle_status
vocabulary (METADATA_STANDARD.md defines: draft, review, approved,
published, superseded, deprecated, retired). This extension is justified
because SDLC delivery scaffold files (templates and agent contracts) are
structural definitions consumed by downstream workflows, not workflow
output instances. They require a distinct lifecycle status to distinguish
them from delivery document instances that flow through the
draft -> changes_requested -> approved state machine. This extension does
not redefine or conflict with any Layer 1 baseline value.
...
1. If review identifies fixable defects, status returns to `draft` for
   refinement.
2. The refine loop has a maximum iteration budget (default: 2,
...
|---|---|---|
| Draft Initiative | DRAFT-INIT | DRAFT-INIT-20260806-001_add-auth-feature.md |
| Initiative | INIT | INIT-20260806-001_add-auth-feature.md |
| Requirement | REQ | REQ-20260806-001_add-auth-feature.md |
| Plan | PLAN | PLAN-20260806-001_add-auth-feature.md |
| Backlog | BACKLOG | BACKLOG-20260806-001_add-auth-feature.md |
| Task | TASK | TASK-20260806-001-01_add-auth-feature.md |
| Implementation | IMPL | IMPL-20260806-001-01_add-auth-feature.md |
| Validation | VALID | VALID-20260806-001_add-auth-feature.md |
| Review | REV | REV-20260806-001_add-auth-feature.md |
| Memory | MEM | MEM-20260806-001_add-auth-feature.md |
| Closure | CLOSE | CLOSE-20260806-001_add-auth-feature.md |
...
Every document that has passed through the technical_critique and
address_critique steps MUST include a Critique Resolution section
documenting:

- Each finding from the technical critique.
- How the finding was addressed.
- The resolution status (resolved, deferred with justification, or
