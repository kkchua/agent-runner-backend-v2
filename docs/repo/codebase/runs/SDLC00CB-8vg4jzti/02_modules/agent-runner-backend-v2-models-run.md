---
title: "Module Documentation: agent_runner_backend_v2.models.run"
template_id: "CB-02"
version: "1.0.0"
doc_type: "system"
authority: "workflow-generated"
scan_policy: "include"
lifecycle_status: "approved"
module_path: "agent_runner_backend_v2/models/run.py"
module_area: "models"
documentation_mode: "summary"
owner_doc_path: "docs/repo/codebase/current/02_modules/agent-runner-backend-v2-models-run.md"
last_verified_by_change: "sdlc_00_codebase_v1 / SDLC00CB-8vg4jzti / 2026-08-05T23:33:46+08:00"
created: "2026-08-05T23:33:46+08:00"
owner: "sdlc_00_codebase_v1"
---

# Module Documentation: agent_runner_backend_v2.models.run

## 1. Module Overview

### 1.1 Purpose

Workflow run and step-run execution models.

### 1.2 Responsibility

This module belongs to the `models` area and is documented as `summary`.

### 1.3 Dependencies

| Dependency | Type | Purpose |
|------------|------|---------|
| `__future__` | stdlib module | imported dependency |
| `datetime` | stdlib module | imported dependency |
| `uuid` | stdlib module | imported dependency |
| `agent_runner_backend_v2.database` | internal module | repository dependency |
| `sqlalchemy` | external module | repository dependency |
| `sqlalchemy.dialects.postgresql` | external module | repository dependency |
| `sqlalchemy.orm` | external module | repository dependency |

## 2. Public API

### 2.1 Classes

#### WorkflowRun

**Inherits from**: `Base`

**Purpose**: A single execution of a workflow with V2 two-field state machine.

#### WorkflowStepRun

**Inherits from**: `Base`

**Purpose**: A single step execution within a workflow run.

#### WorkflowArtifact

**Inherits from**: `Base`

**Purpose**: An output artifact produced by a workflow run or step.

#### WorkflowEvent

**Inherits from**: `Base`

**Purpose**: An immutable event logged during a workflow run lifecycle.

#### WorkflowReview

**Inherits from**: `Base`

**Purpose**: A human or automated review decision recorded against a step run.


### 2.2 Functions

#### utcnow()

**Signature**: `utcnow()`

**Purpose**: Return the current UTC time as a naive datetime.

**Returns**: `datetime`

---


### 2.3 Constants / Configuration

No public constants.


## 3. Error Handling

No documented exceptions.


## 4. Testing

### 4.1 Test Coverage

| Test File | Coverage Area |
|-----------|---------------|
| `tests/conftest.py` | `agent_runner_backend_v2.models.run` |


## 5. Change Log

| Date | Change | Verified By |
|------|--------|-------------|
| 2026-08-05 | Initial baseline generated from repository scan | sdlc_00_codebase_v1 |
